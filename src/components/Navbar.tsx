...
const Navbar: React.FC = () => {
  ...
  const debounceSearchInput = useDebouncedCallback((value: string) => {
    ...
    <nav className="relative">
      <div className="fixed w-full h-[74px] bg-black z-10 flex justify-center inset-0">
        <div className="w-full h-full max-w-[1238px] py-4 px-3 flex justify-between items-center">
          <div className="flex w-2/3 gap-2 max-md:flex-col max-md:w-fit">
            ...
            <div className="flex items-center gap-2 w-fit max-md:hidden">
              <div className="flex items-center z-10">
                <div
                  className={`p-3 bg-zinc-800 rounded-full transition-all duration-150 ${cn(
                    showSearchInput
                ...
                <InputGroup
                  className={`w-full transition-all duration-150 ${cn(
                    showSearchInput
                      ? "max-w-[360px] opacity-100"
                      : "max-w-[0] w-0 opacity-0 hidden"
                  )}`}
                  bg="zinc.800"
                  rounded="full"
                >
              ...
              <Link
                href="/"
                className={
                  "flex items-center gap-2 bg-zinc-800 px-3 py-2 rounded-full z-20 " +
                  cn(pathname === "/" ? "border border-zinc-400" : "")
                }
              >
                <SquaresFour
                  weight="fill"
              ...
              {isLoggedIn && (
                <>
                  <Link
                    href="/profile?tabs=2"
                    className={
                      "flex items-center gap-2 bg-zinc-800 px-3 py-2 rounded-full " +
                      cn(
            ...
            {isLoggedIn && pageLoaded && (
              <>
                ...
                <div className="bg-zinc-800 rounded-full px-3 py-2 md:flex justify-center items-center hidden">
                  <p className="text-base font-medium bg-primary-gradient bg-clip-text text-transparent">
                    {user?.point || 0} SP
                  </p>
                </div>
                <div
                  className="bg-zinc-800 rounded-full px-3 py-2 flex justify-center items-center gap-2 cursor-pointer transition-transform duration-200 active:scale-95"
          ...
          <div
            className="lg:hidden p-3 bg-white rounded-full"
            role="button"
      ...
      <div
        className={`absolute min-h-screen top-0 bottom-0 backdrop-blur-md left-0 right-0 z-50 lg:hidden ${cn(
          showMenuMobile ? "max-md:block" : "max-md:hidden"
        )}`}
      >
        <div
          className="bg-zinc-950  flex flex-col gap-y-3 p-2 pb-8"
          ref={menuRef}
        >
          ...
          <Link
            href="/"
            className={"flex items-center gap-2 bg-zinc-800 p-3 rounded-full"}
          >
            <SquaresFour
              weight="fill"
              color="#A1A1AA"
          ...
          {isLoggedIn && (
            <>
              <Link
                href="/profile?tabs=2"
                className={
                  "flex items-center gap-2 bg-zinc-800 p-3 rounded-full"
                }
              >
                <Coins
                  weight="fill"
              ...
              <div
                className={
                  "flex items-center gap-2 bg-zinc-800 p-3 rounded-full cursor-pointer"
                }
                onClick={() => doLogout()}
              >
                <SignOut
